...
import org.ziglang.plugins.lang.psi.ZigBlock
import org.ziglang.plugins.lang.psi.ZigFnDeclaration
import org.ziglang.plugins.lang.psi.ZigSymbol
	...
	override fun execute(element: PsiElement, resolveState: ResolveState): Boolean {
		if (element is ZigSymbol) {
			val (icon, value, tail, type) = when {
				element.isFunctionName -> quadOf(ZigIcons.icon, element.text, "()", "")
				element.isVariableName ||
						element.isParameter -> quadOf(ZigIcons.icon, element.text, "", "")
				else -> return true
			}
